...
_lr_productions = [
  ...
  ('defdefs -> defdef defdefs','defdefs',2,'p_defdefs_defdef_defdefs','parser.py',27),
  ('defdefs -> defdef','defdefs',1,'p_defdefs_defdef','parser.py',32),
  ('defdef -> DEF ID LPAREN parmsopt RPAREN COLON type BECOMES LBRACE vardefsopt defdefsopt expras RBRACE','defdef',13,'p_defdef_def_id','parser.py',37),
  ('parmsopt -> parms','parmsopt',1,'p_parmsopt_parms','parser.py',42),
  ('parmsopt -> <empty>','parmsopt',0,'p_parmsopt','parser.py',47),
  ('parms -> vardef COMMA parms','parms',3,'p_parms_vardef_comma_parms','parser.py',52),
  ('parms -> vardef','parms',1,'p_parms_vardef','parser.py',57),
  ('vardef -> ID COLON type','vardef',3,'p_vardef_id_colon_type','parser.py',62),
  ('type -> INT','type',1,'p_type_int','parser.py',67),
  ('type -> LPAREN typesopt RPAREN ARROW type','type',5,'p_type_laparen_typesopt_rparen_arrow_type','parser.py',72),
  ('typesopt -> types','typesopt',1,'p_typesopt_types','parser.py',77),
  ('typesopt -> <empty>','typesopt',0,'p_typesopt','parser.py',82),
  ('types -> type COMMA types','types',3,'p_types_type_COMMA_types','parser.py',87),
  ('types -> type','types',1,'p_types_type','parser.py',92),
  ('vardefsopt -> VAR vardef SEMI vardefsopt','vardefsopt',4,'p_vardefsopt_var_vardef_semi_vardefsopt','parser.py',97),
  ('vardefsopt -> <empty>','vardefsopt',0,'p_vardefsopt','parser.py',102),
  ('defdefsopt -> defdefs','defdefsopt',1,'p_defdefsopt_defdef','parser.py',107),
  ('defdefsopt -> <empty>','defdefsopt',0,'p_defdefsopt','parser.py',112),
  ('expras -> expra SEMI expras','expras',3,'p_expras_expra_semi_expras','parser.py',117),
  ('expras -> expra','expras',1,'p_expras_expra','parser.py',122),
  ('expra -> ID BECOMES expr','expra',3,'p_expra_id_becomes_expr','parser.py',127),
  ('expra -> expr','expra',1,'p_expra_expr','parser.py',132),
  ('expr -> IF LPAREN test RPAREN LBRACE expras RBRACE ELSE LBRACE expras RBRACE','expr',11,'p_expr_if_lparen_test','parser.py',137),
  ('expr -> term','expr',1,'p_expr_term','parser.py',142),
  ('expr -> expr PLUS term','expr',3,'p_expr_plus_term','parser.py',147),
  ('expr -> expr MINUS term','expr',3,'p_expr_minus_term','parser.py',151),
  ('term -> factor','term',1,'p_term_factor','parser.py',156),
  ('term -> term STAR factor','term',3,'p_term_star','parser.py',161),
  ('term -> term SLASH factor','term',3,'p_term_slash','parser.py',166),
  ('term -> term PCT factor','term',3,'p_term_pct','parser.py',171),
  ('factor -> ID','factor',1,'p_factor_id','parser.py',176),
  ('factor -> NUM','factor',1,'p_factor_num','parser.py',181),
  ('factor -> LPAREN expr RPAREN','factor',3,'p_factor_lparen_expr_rparen','parser.py',186),
  ('factor -> factor LPAREN argsopt RPAREN','factor',4,'p_factor_factor_lparen_argsopt_rparen','parser.py',191),
  ('test -> expr NE expr','test',3,'p_test_expr_ne_expr','parser.py',196),
  ('test -> expr LT expr','test',3,'p_test_expr_lt_expr','parser.py',201),
  ('test -> expr LE expr','test',3,'p_test_expr_le_expr','parser.py',206),
  ('test -> expr GE expr','test',3,'p_test_expr_ge_expr','parser.py',211),
  ('test -> expr GT expr','test',3,'p_test_expr_gt_expr','parser.py',216),
  ('test -> expr EQ expr','test',3,'p_test_expr_eq_expr','parser.py',221),
  ('argsopt -> args','argsopt',1,'p_argsopt_args','parser.py',226),
  ('argsopt -> <empty>','argsopt',0,'p_argspot','parser.py',231),
  ('expr -> expr COMMA args','expr',3,'p_expr_comma_args','parser.py',236),
  ('args -> expr','args',1,'p_args_expr','parser.py',241),
]
